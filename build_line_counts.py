...
def build_database(transcripts_dir: Path, db_path: Path, rebuild: bool = False):
    ...
    # -----------------------------------------------------------------------
    if rebuild:
        core = ['line_counts', 'characters', 'episodes', 'seasons']

        # Views first: a view over a table blocks nothing, but leaving one
        # behind that references a dropped table gives confusing errors later.
        # Every view here is recreated by this script or by build_keywords.py.
        for (name,) in cur.execute(
                "SELECT name FROM sqlite_master WHERE type='view'").fetchall():
            cur.execute(f"DROP VIEW IF EXISTS {name}")

        # Then any table holding a foreign key into what we are about to drop.
        # With foreign_keys=ON, SQLite refuses to drop a parent that still has
        # children -- which is how adding keyword_counts (it references
        # episodes) broke --rebuild. Discovered rather than hardcoded, so this
        # keeps working as more derived tables are added. All such tables are
        # regenerated by their own build script, so dropping them is lossless.
        for (name,) in cur.execute(
                "SELECT name FROM sqlite_master WHERE type='table'").fetchall():
            if name in core or name.startswith('sqlite_'):
                continue
            parents = {row[2] for row in cur.execute(f'PRAGMA foreign_key_list("{name}")')}
            if parents & set(core):
                cur.execute(f"DROP TABLE IF EXISTS {name}")
                print(f"Dropped dependent table: {name} (rebuild it with its own script)")
    ...
    if columns and 'episode_end' not in columns:
        print("Migrating: adding episodes.episode_end", flush=True)
    ...
    if not season_dirs:
        print(f"Error: No 'Season X' directories found in {transcripts_dir}", flush=True)
        conn.close()
        return
    ...
    for season_dir in season_dirs:
        ...
        print(f"\n{season_dir.name}: {len(txt_files)} files", flush=True)

        for txt_file in txt_files:
            file_info = parse_filename(txt_file.name)
            if not file_info:
                print(f"  Skipping (unrecognised filename): {txt_file.name}", flush=True)
                continue
            ...
            print(f"  {txt_file.name}: {len(line_counts)} characters, {episode_total} lines", flush=True)
    ...
    if orphans:
        print(f"\nRemoved {orphans} stale character(s) with no remaining lines", flush=True)
    ...
    # -----------------------------------------------------------------------
    print(f"\n{'='*60}", flush=True)
    print(f"Database built: {db_path}", flush=True)
    print(f"  Files processed:  {total_files}", flush=True)
    print(f"  Total lines:      {total_lines}", flush=True)
    print(f"  Unique characters: {len(total_characters)}", flush=True)

    # Show top 10 characters by total lines
    print(f"\nTop 10 characters by total line count:", flush=True)
    cur.execute("""
    ...
    for name, total in cur.fetchall():
        print(f"  {total:6d}  {name}", flush=True)
...
def main():
    ...
    print(f"Transcripts directory: {transcripts_dir}", flush=True)
    print(f"Database path:         {db_path}", flush=True)
    print(f"Rebuild:               {args.rebuild}", flush=True)
